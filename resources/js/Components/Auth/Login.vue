...
<template>
    <Dialog :pt="dialogStyles" v-model:visible="show" modal header="Sign in" :style="{ width: '26rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        ...
        <div class="w-full mb-4">
            ...
            <Password :class="$page?.props?.errors?.password && 'p-invalid'" class="w-full p-inputtext-sm" v-model="form.password" toggleMask aria-describedby="text-error" :feedback="false"/>
        ...
        <div class="w-full mb-6 flex justify-between items-center">
            ...
            <Link href="#" class="text-xs text-primary-400">Forgot password ?</Link>
        </div>
        <div class="w-full mb-3">
            <button :disabled="processing" @click="login" class="btn w-full primary">
                Login to account
            </button>
        </div>
